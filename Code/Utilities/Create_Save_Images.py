...
Fsamples=int(int(samples)-Tsamples)
...
for d in range(1,len(data)):
        ...
        if (Track in Seeds)==False:
            Seeds.append(Track)
print(UF.TimeStamp(),bcolors.OKGREEN+'Data has been successfully loaded..'+bcolors.ENDC)
...
for set in range(1,final_count):
  ...
  for Batch in SuperImages:
     if ((set-1)*round((float(totalsamples)/10.0))>len(Batch)):
         continue
