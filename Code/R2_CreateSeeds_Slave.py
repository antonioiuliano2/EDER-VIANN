...
parser.add_argument('--Zmin',help="Minimum Z", default='0')
parser.add_argument('--Zmax',help="Maximum Z", default='0')
parser.add_argument('--Set',help="Set number", default='1')
parser.add_argument('--Subset',help="Subset number", default='1')
parser.add_argument('--SeparationBound',help="Separation Bound", default='4000')
...
Zmin=float(args.Zmin)   #Plate cuts
...
SeparationBound=float(args.SeparationBound)  #The Separation bound is the maximum Euclidean distance that is allowed between hits in the beggining of Seed tracks.

########################################     Preset framework parameters    #########################################
MaxRecords=10000000 #A set parameter that helps to manage memory load of this script (Please do not exceed 10000000
MaxTracks=int(args.MaxTacks)
...
print(UF.TimeStamp(),'Creating seeds... ',bcolors.ENDC)
...
data_header.drop(data_header.index[data_header['z'] >= Zmax], inplace = True)
data_header.drop(data_header.index[data_header['z'] < Zmin], inplace = True)
Records=len(data_header.axes[0])
print(UF.TimeStamp(),'There are total of ', Records, 'tracks in the data set',bcolors.ENDC)
...
StartDataCut=(1-Subset)*MaxTracks
EndDataCut=Subset*MaxTracks

#Specifying the right join
r_data=data.rename(columns={"x": "r_x"})
r_data.drop(r_data.index[StartDataCut:min(EndDataCut,len(r_data.axes[0]))],inplace=True)
...
#Downcasting Panda Data frame types to save memory
...
for i in range(0,Steps):
  ...
  merged_data.drop(merged_data.index[merged_data['separation'] > SeparationBound], inplace = True) #Dropping the Seeds that are too far apart
...
print(UF.TimeStamp(), bcolors.OKGREEN+"Seed creation is finished..."+bcolors.ENDC)
#End of the script
