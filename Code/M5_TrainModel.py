...
parser.add_argument('--ModelName',help="Would you like to create a new model from scratch? If yes, please enter its name (without specifying the directory:", default='Default')
parser.add_argument('--LR',help="Would you like to modify the model Learning Rate, If yes pleasew enter it here eg: 0.01 ", default='Default')
...
if mode=='R':
 UF.CreateTrainModelCleanUp(AFS_DIR, EOS_DIR)
 job=[]
 job.append(1)
 job.append(1)
 ...
 UF.LogOperations(EOSsubModelDIR+'/'+ModelName+'_Performance.csv','StartLog',PerformanceHeader)
 UF.LogOperations(EOSsubModelDIR+'/JobTask.csv','StartLog',[job])
...
if mode=='C':
   ...
   csv_reader=open(EOSsubModelDIR+'/JobTask.csv',"r")
   PreviousJob = list(csv.reader(csv_reader))
   if os.path.isfile(EOSsubModelDIR+'/'+PreviousJob[0][8])==False:
   ...
   required_file_name=EOSsubModelDIR+'/model_train_log_'+PreviousJob[0][0]+'.csv'
   if os.path.isfile(required_file_name)==False:
     ...
     if UserAnswer=='R':
        if CurrentSet==1:
          UF.SubmitTrainJobCondor(AFS_DIR,EOS_DIR,PreviousJob[0],'New')
        if CurrentSet>1:
      ...
      UF.LogOperations(EOSsubModelDIR+'/'+ModelName+'_Performance.csv','UpdateLog',PreviousHeader)
      ...
      if os.path.isfile(EOS_DIR+'/EDER-VIANN/Data/TRAIN_SET/'+'TRAIN_SET_'+str(CurrentSet+1)+'.csv')==False:
      ...
      if UserAnswer=='Y':
          CurrentSet+=1
          PreviousJob[0][0]=str(CurrentSet)
          UF.LogOperations(EOSsubModelDIR+'/JobTask.csv','StartLog',PreviousJob)
          UF.SubmitTrainJobCondor(AFS_DIR,EOS_DIR,PreviousJob[0],'Train')
